...
console.log('sql selecting where '+taskId);
...
// Forum post and reply object relating functions

//SQL query for insert a new post
Utils.prototype.sqlqueryInsertPost =function(Post)
{
    
    var sqlInsertPost = "INSERT INTO posts (post_id,post_author,post_title,post_body,post_category) VALUES ("+
        "NULL,"+
        "'"+Post.postAuthor +"',"+
        "'"+Post.postTitle +"',"+
        "'"+Post.postBody +"',"+
...
Utils.prototype.getSqlSelectPosts =function(){
    var sqlSelectPost = "SELECT * FROM posts";

    return sqlSelectPost;
}

//select posts by postheading

Utils.prototype.getSqlSelectPost =function(Post){
    var sqlSelectPostbyid = "SELECT * FROM posts WHERE post_id='"+ Post.postId;

    return sqlSelectPostbyid;
}

//Delete Post
Utils.prototype.getSqlDeletePost =function(Post){
    
    var sqlDeletePost ="DELETE FROM posts WHERE post_id="+Post.postId;

    return sqlDeletePost;
} 

//search posts by postheading

Utils.prototype.getSqlSearchPosts =function(Post){
    console.log(Post);
    
    var sqlSearchPosts = "SELECT * FROM posts WHERE post_title LIKE'%"+ Post.postTitle+"%'";

    return sqlSearchPosts;
}


/* Insert reply */
Utils.prototype.getSqlInsertReply =function(reply){
    console.log(reply);
    console.log('123');
    
    
    var sqlInsertReply ="INSERT INTO postsreply (reply_id,post_id,reply_author,reply_body) VALUES ("+
    "NULL,"+
    reply.postId+",'"+
    reply.replyAuthor+"','"+
    reply.replyBody+"')";

    return sqlInsertReply;
...
/* select post replys by postID */
Utils.prototype.getSqlSelectReply =function(reply){
    var sqlSelectReply ="SELECT * FROM postsreply WHERE post_id ="+reply.postId;
    
    return sqlSelectReply;
}

//Delete Post reply by reply id 
Utils.prototype.getSqlDeleteReply =function(reply){
    var sqlDeletelPost ="DELETE FROM postsreply WHERE reply_id="+reply.replyId;

    return sqlDeletelPost;
}

//Update post reply by reply id

Utils.prototype.getSqlUpdateReply =function(reply)
{
    var sqlUpdateReply = "UPDATE postsreply SET reply_body='"+reply.replyBody +"' WHERE reply_id="+reply.replyId;

    return sqlUpdateReply;
}
...
Utils.prototype.generateUser = function (resultUser) {
    ...
    console.log( " in generateuser" + resultUser);
    var user = null;
    if (resultUser) {
        user = new User(
            resultUser.user_id,
...
// task object generating function(not working)
Utils.prototype.generateTask = function (resultTask) {
    var task = null;
    console.log( " in generateTask" + resultTask);
    if (resultTask) {
        task = new Task(
            resultTask.task_id,
            resultTask.responsible_student,
            resultTask.archived_date,
            resultTask.supposed_date,
            resultTask.description);
    }

    return task;
    
     console.log( task+ " in util gentask return");

};

// POST object generating function
Utils.prototype.generatePost = function (resultPost) {
    var post = null;
    if (resultPost) {
        post = new Post(
            resultPost.post_id,
            resultPost.post_author,
            resultPost.post_title,
            resultPost.post_body,
            resultPost.post_category,
            resultPost.posted_time);
    }
    return post;
};

// POST object generating function
Utils.prototype.generatepostReply = function (resultpostReply) {
    var postReply = null;
    if (resultpostReply) {
        postReply = new Reply(
            resultpostReply.reply_id,
            resultpostReply.post_id,
            resultpostReply.reply_author,
            resultpostReply.reply_body,
            resultpostReply.replied_time);
    }
    return postReply;
};
